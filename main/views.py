...
def buy(request,id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            print(form.errors) 
            messages.error(request, f'sorry! Fill all fields with valid data')
...
def vendorprofile(request):
    print("User authenticated:1", request.user.is_authenticated)
    if request.user.is_staff:
        ...
        print("User authenticated:2", request.user.is_authenticated)
        if request.method == 'POST':
            print("User authenticated3:", request.user.is_authenticated)
            form = ProductDetailsforms(request.POST, request.FILES)
            print("User authenticated4:", request.user.is_authenticated)
            if form.is_valid():
                print("User authenticated5:", request.user.is_authenticated)
                new_product = form.save(commit=False)  
                new_product.Pvendorname= request.user
                new_product.save() 
